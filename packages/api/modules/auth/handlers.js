...
const getToken = async (req, h) => {
  if (req.payload.grant_type === 'authorization_code') {
    ...
    const user = await User.findOne({ $or: [{ email: providerUser.email }, { authProviders: { $elemMatch: { type: provider, username: providerUser.id } } }] })
...
const revokeToken = async (req, h) => {
  if (!req.auth.credentials) return h.response()
