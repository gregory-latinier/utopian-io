...
export const Steem = {
  ...
  methods: {
    ...
    async steemPost ({ body, permlink, tags, title, url, blockchain, context, category }) {
      try {
        const postingKey = await this._getPostingKey()
        const author = this.getUserSteemAccount()
        const jsonMetadata = JSON.stringify({ app: 'openchange', format: 'markdown', tags })
        const parentPermlink = 'openchange'
        const operations = []
        if (!blockchain) {
    ...
    async transferSteems ({ steem, key, from, to, url }) {
    ...
    async steemPowerTip ({ weight, permlink, author }) {
      const postingKey = await this._getPostingKey()
      const operations = []
      operations.push([
        'vote',
        {
          voter: this.getUserSteemAccount(),
          author,
          permlink,
          weight
        }
    ...
    /**
     * Parse html to MD
     * Append a footer link to redirect users back to openchange
     *
     * @param body {string} - post content
     * @param url {string} - return url to openchange
     *
    ...
    async _getPostingKey () {
      ...
      return aesjs.utils.utf8.fromBytes(decryptedBytes).substr(3).trim()
    }
  },
  watch: {
...
const getBeneficiaries = async ({ context }) => {
  if (context === 'article') {
    return [{ account: 'openchange', weight: 500 }]
  } else if (context === 'bounty') {
    return [{ account: 'openchange', weight: 10000 }]
  } else if (context === 'bounty-solution') {
    return [{ account: 'openchange', weight: 1500 }]
  }
}
