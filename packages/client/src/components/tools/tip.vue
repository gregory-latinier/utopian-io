...
import { decimal, maxValue, minValue, required, requiredUnless } from 'vuelidate/lib/validators'
...
export default {
  ...
  data () {
    return {
      ...
      steemPowerTipInfo: null,
      userTippedInSteemPower: true,
      steemForm: {
        steem: null,
        sbd: null,
        activeKey: null
  ...
  methods: {
    ...mapActions('tips', ['getAuthorInfo', 'hasUserTippedInSteemPower', 'getSteemPowerTipInfo', 'saveTip']),
    resetForm () {
    ...
    async openTipModal () {
      this.resetForm()
      this.modal = true
      this.userTippedInSteemPower = await this.hasUserTippedInSteemPower({
        obj: this.obj,
        id: this.id
      })
    ...
    async selectCurrency (currency) {
      ...
      } else if (this.currency === 'steemPower') {
        this.$v.steemPowerForm.$reset()
      }
    ...
    async nextStep () {
      if (this.step === 'amount') {
        if (this.currency === 'steem') {
          ...
          this.$v.steemPowerForm.$touch()
          if (this.$v.steemPowerForm.$invalid) {
            return
          }
          this.steemSenderUser = this.getUserSteemAccount()
        }
        ...
        if (this.currency === 'steem') {
          ...
          this.steemPowerTipInfo = await this.getSteemPowerTipInfo({
            obj: this.obj,
            id: this.id
          })
          if (this.steemPowerTipInfo === null) {
            this.setAppError('components.tools.tip.errors.tipSteemPowerOnce')
            this.submitting = false
            return
          }
          data = await this.steemPowerTip({
            weight: this.steemPowerForm.weight * 100,
            permlink: this.steemPowerTipInfo.permlink,
            author: this.steemPowerTipInfo.author
          })
          this.steemTransaction = data.id
          tips.push({
            currency: 'steemPower',
            amount: this.steemPowerForm.weight
          })
          this.userTippedInSteemPower = true
        }
        await this.saveTip({
          obj: this.obj,
          id: this.id,
    ...
    q-modal-layout.tip-modal
      ...
      q-stepper(
        ...
        q-step(
          ...
          .row
            ...
            q-btn.col-md-6.currency(
              ...
              span STEEM
            q-btn.col-md-6.currency(
              v-if="steemEnabled && !userTippedInSteemPower"
              flat
              @click="selectCurrency('steemPower')"
            )
              img(src="statics/currencies/steem.svg")
              span STEEM POWER
            ...
            q-btn.col-md-6.currency(
              ...
              q-tooltip {{ $t('components.tools.tip.errors.tipSteemPowerOnce')}}
              img(src="statics/currencies/steem.svg")
              span STEEM POWER
          .row.q-mt-md
            q-btn.col-md-6.currency(
        ...
        q-step(
          ...
          div(v-if="currency === 'steemPower'")
            | {{$t('components.tools.tip.steps.2.steemPowerHelp')}}
            q-slider(
              v-model="steemPowerForm.weight"
              :min="1"
              :max="100"
              :step="1"
              label
        ...
        q-step(
          ...
          .verify-text.q-my-md(
            v-if="currency === 'steemPower'"
            v-html="$t('components.tools.tip.steps.3.steemPowerInfo', { sp: steemPowerForm.weight, senderSteem: steemSenderUser, author: username })"
          )
          div
        ...
        q-step(
          ...
          .steem-transaction.flex.column(v-if="currency === 'steem' || currency === 'steemPower'")
