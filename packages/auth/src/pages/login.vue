...
export default {
  ...
  methods: {
    startGithubLogin () {
      ...
      window.location = `https://github.com/login/oauth/authorize?scope=read:user,user:email&client_id=${process.env.GITHUB_CLIENT_ID}&state=${state}`
    },
    startGoogleLogin () {
...
<template lang="pug">
.row.flex.items-center.justify-center.connect-form
  .col-md-6.col-sm-12.col-xs-12.inner-form
    div.connect
      .q-headline.q-mb-lg {{ $t('auth.login.welcome') }}
      .q-subheading.q-mb-lg {{ $t('auth.login.text') }}
      ...
      .q-body-1.text-grey.q-mb-lg {{ $t('common.acceptance') }}
      <a class="q-body-1" :href="`/${$route.params.locale}/tos`">{{ $t('common.tos') }}</a> -
      <a class="q-body-1" :href="`/${$route.params.locale}/privacy`">{{ $t('common.privacyPolicy') }}</a>
</template>
